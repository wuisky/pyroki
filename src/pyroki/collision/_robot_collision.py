...
class RobotCollision:
    ...
    def compute_self_collision_distance(
        ...
        assert coll.get_batch_axes() == (*batch_axes, self.num_links) or len(coll.get_batch_axes()) == len(batch_axes) + 1
    ...
    def compute_world_collision_distance(
        ...
        has_nested_dims = len(coll_batch_axes) > len(batch_cfg_shape) + 1        # Handle nested batch dimensions (e.g., multiple spheres per link)
        if has_nested_dims:
            # Flatten nested dimensions: (*batch, num_links, num_spheres) -> (*batch, num_links*num_spheres)
